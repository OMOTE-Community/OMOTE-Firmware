...
void notify_active_tabs_before_delete(t_gui_state *gui_state) {
  ...
  for (int index=0; index <= 2; index++) {
    if (gui_state->gui_on_tab[index].gui_list_index == -1) {
      omote_log_d("    Will not notify tab %d about deletion because it does not exist", index);
      continue;
    }
    ...
    if (nameOfTab == "") {
      omote_log_w("    Will not notify tab %d about deletion because it is not set", index);
    } else if (registered_guis_byName_map.count(nameOfTab) == 0) {
      omote_log_w("    Can not notify tab %d about deletion because name \"%s\" was not found in registry", index, nameOfTab.c_str());
    } else {
      omote_log_d("    Will notify tab %d with name \"%s\" about deletion", index, nameOfTab.c_str());
...
void create_new_tab(lv_obj_t* tabview, t_gui_on_tab *gui_on_tab) {
  ...
  if (nameOfTab == "") {
    ...
    omote_log_w("    Will not create new tab because name %s was not found in registry", nameOfTab.c_str());
  } else {
    omote_log_d("    Will create tab with name \"%s\" ", nameOfTab.c_str());
...
void doTabCreation_strategyMax3(lv_obj_t* tabview, t_gui_state *gui_state) {
  ...
  // create the tabs
  omote_log_d("  Will create tabs. List indices of the three tabs are %d, %d, %d, tab nr %d will be activated", gui_state->gui_on_tab[0].gui_list_index, gui_state->gui_on_tab[1].gui_list_index, gui_state->gui_on_tab[2].gui_list_index, gui_state->activeTabID);
  for (int i=0; i<3; i++) {
  ...
  if (get_gui_list_active_withFallback()->size() > 0) {
    ...
    omote_log_d("  New visible tab is \"%s\"", nameOfNewActiveTab.c_str());
...
void gui_memoryOptimizer_onStartup(lv_obj_t** tabview, lv_obj_t** panel, lv_obj_t** img1, lv_obj_t** img2) {
  ...
  omote_log_i("Startup: try to resume at scene \"%s\" with GUI \"%s\"", gui_memoryOptimizer_getActiveSceneName().c_str(), gui_memoryOptimizer_getActiveGUIname().c_str());
  ...
  for (int i=0; i<get_gui_list_active_withFallback()->size(); i++) {
    if (get_gui_list_active_withFallback()->at(i) == gui_memoryOptimizer_getActiveGUIname()) {
      omote_log_i("Startup: found GUI with name \"%s\" in \"gui_list_active\" at position %d", gui_memoryOptimizer_getActiveGUIname().c_str(), i);
      gui_list_index = i;
      break;
    }
  }
  ...
  if ((gui_list_index >= 0) && (gui_list_index < get_gui_list_active_withFallback()->size())) {
    ...
    // gui was not found
    omote_log_w("Startup: GUI with name \"%s\" was not found. Will start with first GUI of main_gui_list", gui_memoryOptimizer_getActiveGUIname().c_str());
...
void gui_memoryOptimizer_navigateToGUI(lv_obj_t** tabview, lv_obj_t** panel, lv_obj_t** img1, lv_obj_t** img2, GUIlists GUIlist, int gui_list_index) {
  ...
  if ( !((gui_list_index >= 0) && (gui_list_index < get_gui_list_withFallback(GUIlist)->size()))) {
    omote_log_w("  cannot navigate to GUI because gui_list_index \"%d\" is out of range", gui_list_index);
    return;
  }  
